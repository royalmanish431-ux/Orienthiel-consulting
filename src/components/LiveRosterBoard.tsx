...
export const LiveRosterBoard: React.FC<LiveRosterBoardProps> = ({ onSelectRole }) => {
  ...
  return (
    <div 
      ...
      className="bg-[#131B2E] text-[#F2F3EF] rounded-lg p-5 sm:p-6 shadow-2xl border border-white/10 relative overflow-hidden group"
    >
      ...
      <div className="flex items-center justify-between border-b border-white/15 pb-4 mb-4 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono bg-white/10 text-white/80 px-2 py-0.5 rounded">
            v2.6
          </span>
        </div>
      ...
      {/* Board Ticker Rows */}
      <div className="space-y-1 relative z-10" id="board-rows-container">
        {SPLIT_FLAP_SEEDS.map((item, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Vertical Badge */}
              <div>
                <span className={`font-mono text-[10.5px] uppercase tracking-wider px-2 py-0.5 rounded-sm inline-block text-center border ${verticalStyles}`}>
              ...
              {/* Status */}
              <div className="text-right flex items-center justify-end gap-1.5 font-mono text-[11px] text-white/55">
